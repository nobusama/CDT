...
Version integrating pre-trained foundation models:
...
class SimpleTransformerEncoder(nn.Module):
    """
    Simple Transformer same as Phase 2
    Will be replaced with RNA-FM/ESM-2 in Phase 4
    """
...
class CDTModelPhase3(nn.Module):
    """
    CDT Model Phase 3: Pre-trained Models Integration Version
    ...
    2. Projection Layers (convert to unified dimension)
    3. Cross-Attention Layers
    4. Virtual Cell Embedder

    Week 2: DNA (NT-v2) integration - done
    Week 3: Replace with RNA (RNA-FM) - done
    Week 4: Replace with Protein (ESM-2) - done
    """
    ...
    def __init__(
        ...
        """
        Args:
            dna_model_name: Hugging Face DNA model name
            freeze_dna: Whether to freeze DNA encoder
            freeze_rna: Whether to freeze RNA encoder
            protein_model_name: ESM-2 model name
            freeze_protein: Whether to freeze Protein encoder
            d_model: Unified model dimension
            nhead: Number of Cross-Attention heads
        ...
        # DNA output dimension: 512 -> d_model (768)
        ...
        # RNA output dimension: 640 -> d_model (768)
        rna_hidden_size = 640  # RNA-FM output dimension
        ...
        # Protein output dimension: 640 -> d_model (768)
        protein_hidden_size = 640  # ESM-2 output dimension
        self.protein_projection = nn.Linear(protein_hidden_size, d_model)

        print(f"  Protein: {protein_hidden_size}d -> {d_model}d (projection)")

        # =================================================================
        # 4. Cross-Attention Layers (same as Phase 2)
        # =================================================================
        ...
        # =================================================================
        # 5. Virtual Cell Embedder (same as Phase 2)
        # =================================================================
    ...
    def forward(
        ...
        Args:
            dna_sequences: List[str] list of DNA sequence strings
                e.g.: ["ATGCGATCG", "ATGGCTA"]

            rna_sequences: List[str] list of RNA sequence strings
                e.g.: ["AUGCUAGCU", "AUGGCUA"]

            protein_sequences: List[str] list of Protein sequence strings
                e.g.: ["MKTAYIAK", "MKTAY"]

            return_attention: Whether to return attention maps

        Returns:
            cell_embedding: [batch, d_model] cell-level embeddings
        ...
        # Match device (run on CPU)
        device = torch.device('cpu')  # Unified to CPU
        dna_inputs = {k: v.to(device) for k, v in dna_inputs.items()}

        # Extract features with Nucleotide Transformer v2
        with torch.no_grad():
        ...
        # DNA padding mask (invert transformers' 1=valid, 0=padding)
        ...
        # RNA padding mask (RNA-FM handles padding automatically)
        rna_padding_mask = None  # RNA-FM handles internally
        ...
        # Protein padding mask (ESM-2 handles internally)
        protein_padding_mask = None  # ESM-2 handles internally
